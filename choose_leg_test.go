...
// The spread is held as an int64 and used to be narrowed to int on the way in.
// int is 32 bits on mips and arm32, so the same config file meant one threshold
// there and another on amd64. Whatever the word size, a number this size cannot
// be taken at face value.
func TestChooseLegSpreadIsBoundedOnEveryWordSize(t *testing.T) {
	// 2^32+10, which read through a 32 bit int is a spread of 10 percent.
	const wide = int64(1)<<32 + 10

	legs := []probeLeg{leg("10.0.0.1", 10), leg("", 3000)}
	got, ok := chooseLeg(legs, wide, 1)
	require.True(t, ok)
	assert.Equal(t, netip.MustParseAddr("10.0.0.1"), got.relay,
		"a direct path three hundred times slower is not an equal")
}

// The cutoff is the fastest path scaled by the percentage, in nanoseconds. A
// round reports up to pathProbeWait, and three seconds times a ten digit
// percentage does not fit in an int64: the product wrapped negative, every path
// fell outside the cutoff, and the spread stopped happening at all.
func TestChooseLegSpreadDoesNotOverflow(t *testing.T) {
	const wide = int64(1)<<32 + 10

	legs := []probeLeg{leg("10.0.0.1", 3000), leg("10.0.0.2", 3100)}
	seen := map[netip.Addr]int{}
	for seed := uint64(0); seed < 100; seed++ {
		got, ok := chooseLeg(legs, wide, seed)
		require.True(t, ok)
		seen[got.relay]++
	}
	assert.Len(t, seen, 2, "two paths a tenth of a second apart must still be shared out")
}
