...
const (
	...
	// maxPathProbeSpread is the largest spread that still leaves a probe worth
	// running. The cutoff sits that percentage above the fastest path, so 900
	// would call anything up to ten times slower an equal, and since a direct
	// path wins a tie it would win every one of them: the measurement would
	// then decide nothing at all. Half again as slow is already a stretch for
	// two paths called equal, and it keeps the nanosecond arithmetic in
	// chooseLeg far from the end of an int64.
	maxPathProbeSpread = 50
)
...
//
// The spread is capped at maxPathProbeSpread; see there for why a bigger one
// cannot be honoured.
func chooseLeg(legs []probeLeg, spreadPct int64, seed uint64) (probeLeg, bool) {
	var best probeLeg
	found := false
	for _, l := range legs {
		if !l.got {
	...
	// The config reader already warns about a spread over the limit and brings
	// it down, but the arithmetic is here, so the bound is kept here as well: a
	// ten digit percentage makes the nanosecond product below wrap negative,
	// and a negative cutoff excludes every path without a sign that it did.
	spreadPct = min(spreadPct, maxPathProbeSpread)
